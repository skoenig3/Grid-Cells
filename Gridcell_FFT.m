...
numreps = 100; %number of shuffled autocorrelations. Could do many less but e.g. 10 to speed up
...
% but smooths over the kernel range after binninga so is potentially less precise
...
for reps = 1:numreps
    ...
    sh = reshape(h(shuffind),numrow,numcol); %shuffle time and spikes together
    st = reshape(t(shuffind),numrow,numcol);
    
    sh2 = imfilter(sh,H,o.imfiltType);
    st2 = imfilter(st,H,o.imfiltType);
    
    sshortind = st2<o.tth; % USE SMOOTHED TIMES FOR THRESHOLDING
    sh2(sshortind) = nan;
    st2(sshortind) = nan;
    
    shuffled_rs{reps} = sh2./st2; %shuffled firing rate maps
end
...
%% Taper and buffer observed autocorrelation 
...
%% Calculate shuffled autocorrelation functions and their power spectrum
...
%% Calculate average expected power spectrum of shuffled autocorrelations
...
%could use mean,std,confidence intervals etc. but all give similar answers

%% Remove shuffled autocorrelation power from observed power

cmag = mag.^2-avg_shuffled_mag;
cmag(cmag < 0) = 0;
figure
imagesc(cmag)
box off
...
%% Make the plots and determine peak frequencies & orientations
...
subplot 333;
imagescnan(x_freq_axis(fsel),x_freq_axis(fsel),cmag(fsel,fsel));
